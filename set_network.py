...
def b_l_1(borrower,lenders_l,b_l_dic,count):
    print "b"+borrower
    bld = pd.DataFrame(columns=["borrowerkey", "lenderkey", "success_bid_time", \
                                                    "all_bid_time", "listing_time", "win_listing_time",
                                                    "withdraw_bid_time", "success_bid_amount", \
                                                    "success_bid_percent_b", "B_funded_listing_num", "B_loan_num",
                                                    "B_loan_paid", "B_loan_notpaid"])
    ...
    # pd.DataFrame.to_csv(bld, path_or_buf="processed_datas/"+borrower+str(count)+".csv",
    #                         index=False)
    print count
    res=open("processed_datas/b_l/"+str(count)+".pkl","wb")
    pickle.dump(bld,res)
    res.close()
...
bld = pd.DataFrame(columns=["borrowerkey", "lenderkey", "success_bid_time", \
                                ...
                                "success_bid_percent_b", "B_funded_listing_num", "B_loan_num",
                                "B_loan_paid", "B_loan_notpaid"])
for _,borrower in enumerate(borrowers_l[300:500]):
    print "b" + borrower
    ...
    b_l_dic["B_loan_num"] = b_l_b_loan_data.shape[0]
    b_l_dic["B_loan_paid"] = b_l_b_loan_data[b_l_b_loan_data["loanstatus"] == "Paid"].shape[0]
    b_l_dic["B_loan_notpaid"] = b_l_b_loan_data[(b_l_b_loan_data["loanstatus"] == "Defaulted (Bankruptcy)") | (
    b_l_b_loan_data["loanstatus"] == "Defaulted (Delinquency)") | \
                                                (b_l_b_loan_data["loanstatus"] == "Defaulted (PaidInFull)") | (
                                                b_l_b_loan_data["loanstatus"] == "Defaulted (SettledInFull)")].shape[0]
    borrower_bid_amount = b_l_b_data[b_l_b_data["bidstatus"] == "Winning"].loc[:, "participationamount"].sum()
    for lender in lenders_l:
        b_l_dic["lenderkey"] = lender
        b_l_data = b_l_b_data[b_l_b_data["lenderkey"] == lender]
    ...
    if _<3:
        pd.DataFrame.to_csv(bld, path_or_buf="processed_datas/" + borrower + str(_ + 300) + ".csv",
                            index=False)

    print _
    res = open("processed_datas/b_l/" + str(_ + 300) + ".pkl", "wb")
    pickle.dump(bld, res)
    res.close()
    bld.drop(bld.index,inplace=True)
...
#     def __init__(self,borrower,lenders_l,b_l_dic,_):
#         threading.Thread.__init__(self)
#         self.borrower=borrower
#         self.lenders_l=lenders_l
#         self.b_l_dic=b_l_dic
#         self.count=_
#
#     def run(self):
#         # 获得锁，成功获得锁定后返回True
...
#         # 否则超时后将返回False
#         #threadLock.acquire()
#         b_l_1(self.borrower, self.lenders_l, self.b_l_dic,self.count)
#         # # 释放锁
#         #threadLock.release()
#
# threads_1=[]
# for _,borrower in enumerate(borrowers_l[2:1000]):
#     t = threading.Thread(target=b_l_1, args=(borrower,lenders_l,b_l_dic,_))
#     threads_1.append(t)
#     print borrower + "t"
#
# # threads_2=[]
...
# # print borrowers_to_lenders_df.shape[0]
# # print borrowers_to_lenders_df.head()
